...
class Captcha
{
    ...
    public function getCode($token,$id = ''){
        $code = Cache::get("captcha_{$token}_{$id}");
        return $code;
    }
     ...
     * @return bool
     */
    public function checkWordCode($code,$token,$id = ''){
        $result = Cache::get("captcha_{$token}_{$id}");
        $word = isset($result['verify_word']) ? implode("",$result['verify_word']) : '';
        Cache::set("captcha_{$token}_{$id}",null);
        if($word == $code){
            Cache::set("captcha_{$token}_check",time(),1800);
            return true;
        }
        return false;
    }
     ...
     * 图片验证码是否校对成功
     */
    public static function checkWordCodeValid($token){
        $time = Cache::get("captcha_{$token}_check");
        Cache::set("captcha_{$token}_check",null);
        if($time){
           return true;
        }
        return false;
    ...
    public function entryWord($token,$id = ''){
        $this->seKey = $token;
        // 图片宽(px)
        $this->imageW || $this->imageW = $this->length * $this->fontSize * 1.5 + $this->length * $this->fontSize / 2;
        // 图片高(px)
        ...
        // 保存验证码
        $secode                = [];
        $secode['verify_word'] = $code; // 把校验码保存到session
        $secode['verify_code'] = $this->authcode(strtoupper(implode('', $code))); // 把校验码保存到session
        $secode['verify_time'] = time(); // 验证码创建时间
        //Session::set($key . $id, $secode, '');
        Cache::set("captcha_{$token}_{$id}",$secode,1800);
    ...
    private function getFont(){
        ...
        $dir->close();
        //var_dump($ttfPath.$ttfs[array_rand($ttfs)]);
        return $ttfPath.$ttfs[array_rand($ttfs)];
    }
}