...
class Loader
{
     ...
     */
    public static function model($name = '', $layer = 'model', $appendSuffix = false, $common = 'common',$database = null)
    {
        $guid = $name .$database. $layer;
        ...
        if (class_exists($class)) {
            ...
            Log::write('model $class test :11'.$class);
            if (class_exists($class)) {
                $model = new $class();
            } elseif($database) {
                //$class = self::parseClass($module, $layer, $name, $appendSuffix);
                ...
                Log::write('model $class test :00'.$class);
                $className = self::parseName($name,1) . (App::$suffix || $appendSuffix ? ucfirst($layer) : '');
                $class .= $className;
                if (!class_exists($class)) {
                    self::_createModel($name,$layer,$appendSuffix,$common,$database);
                }
                try {
                    $model = new $class();
     ...
     * @param $database
     */
    private static function _createModel($name,$layer,$appendSuffix,$common,$database){
        $path = ROOT_PATH."library/model/";
...
class ' . $name . ' extends Model {
    ...
    public function _setModelConfig(){
        '.($database && $database!= 'main' ? '$this->connection = config("db.'.$database.'");':'$this->connection = config("database");').'
    }
}';
        swritefile($path."{$name}.php",$writetext);
    }
