...
function getTeam($team_id, $key = null, $field = []) {
    ...
    $team['logo_big'] = get_image_thumb_url($team['logo']);
    if (!is_file($team['logo_big'])){
        $team['logo_big'] = get_image_thumb_url_logo($team['logo']);
    }
    //$team['logo'] = get_image_thumb_url('',90,90);
    $team['logo'] = get_image_thumb_url($team['logo'], 90, 90);
    Log::write('$team $team --is_file-- :'.is_file($team['logo']));
    if (!is_file($team['logo'])){
        $team['logo'] = $team['logo_big'];
        Log::write('$team $team --test test test--');
        $team['logo1'] = get_image_thumb_url_logo($team['logo']);
        Log::write('$team $team --test test test-- :'.json_encode($team['logo1']));
    }
    Log::write('$team $team ---- :'.json_encode($team));
    if ($field) {
        $data = [];
...
function get_image_thumb_url_logo($sourceImageURL, $maxWidth = 0, $maxHeight = 0, $type = 3) {
    ...
    $url = $url . "{$dirname}{$fileName}.{$extension}"; //;http://res.51bet.com/attach/team_logo/c8/75/02/1911471319504122_848172f315_2_90_90.jpg
...
function errPage($message, $title = '', $back = '') {
    ...
    return view("../application/library/view/mobile_error.html", $vars, [], 404);
}
