...
//      //          $val['logo_hover'] = get_image_thumb_url($val['logo_hover']);
    ...
    public function msglist()
    {
        ...
        //$page = $last_id?$last_id:1;
        $limit = 8;
        $offset = $last_id;
        //$where = array("type" => 2);//"id" => $last_id,
        //$data = Db::name('help')->where($where)->limit($offset, $limit)->select();
        $data = Db::name('notice')->limit($offset, $limit)->select();
        Log::write('msglist $data :'.json_encode($data));
        Log::write('msglist $data :'.Db::name('notice')->getLastsql());
