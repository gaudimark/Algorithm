...
class Play extends Safe
{
    ...
    public function all()
    {
        ...
        //$test = input('test',0);
        $hot = input('hot');//1-冷门，2-普通，3-热门
        $recommend = input('recommend');// 0 默认   1 推荐
        ...
        if ($recommend) {
            $where['p.is_recommend'] = $recommend;
        }
...
//        }
        ...
        $lists = Db::name('play')->alias("p")
            ...
            ->group('o.play_id')->limit($offset, $limit)
            ->select();
        //echo Db::name('play')->getLastSql();exit;
        //echo Db::name('play')->getLastSql();
        Log::write('play getLastSql :'.Db::name('play')->getLastSql());
        $playSvr = new \library\service\Play();
        /* $sqlStr = Db::name('play')->getLastSql();
         $total = cache("index_play_list_total_" . md5($sqlStr));
         if (!$total) {
         ...
         $total_pages = ceil($total / $limit); */
        Log::write('play all $lists: '.json_encode($lists));
        $matchs = [];
        $tabname = [100];
        if ($lists) {
            foreach ($lists as $key => $val) {
               ...
               // }
                ...
                Log::write('play all $teams: '.json_encode($lists));
                if (isset($matchs[$val['match_id']])) {
                    $match = $matchs[$val['match_id']];
                } else {
                    $match = getMatch($val['match_id'], null, ['name']);
                ...
                // tab  name
                // 热门
                if ($val['hot'] == 3 && !in_array(101, $tabname)){
                    $tabname[] = 101;
                }
                // 推荐
                if ($val['is_recommend'] == 1 && !in_array(101, $tabname)){
                    $tabname[] = 102;
                }
                Log::write('play all $match: '.json_encode($match));
                // 赛事
                /*
                [100] = "全部比赛",
                [101] = "热门比赛",
                ...
                [108] = "阿甲"
                */
                $matchids = [1 => 107,19 => 102,53 => 103,49 => 104,41 => 105,210 => 106,178 => 108];
                if (array_key_exists($val['match_id'], $matchids) && !in_array($matchids[$val['match_id']], $tabname)){
                    $tabname[] = [$matchids[$val['match_id']],$val['match_id']];
                }
        ...
        Log::write('play all $lists detail: '.json_encode($lists));
        $data = array_values($lists);
        //$data[]['tabname'] = $tabname;
        Log::write('play all $data: '.json_encode($data));
        if (!$team_name) {
            Cache::hSet('match_list', $matchId, json_encode($data));
        }
        return $this->retSucc('play.all', $data, $tabname);
    }
    ...
    public function arena()
    {
        ...
        //获取擂台列表
        $field = "id,mark,has_sys,rules_type,rules_id,game_type,min_bet,max_bet,match_id,game_id,user_id,odds,status,private,deposit,intro,bet_money,create_time";
        if ($nickname) {
        ...
        $total_pages = ceil($total / $limit);
        $next_page = $total_pages > $page ? 1 : 0;
        Log::write('arena info :'.json_encode($info));
        //\think\Debug::remark("end");
        //echo \think\Debug::getRangeMem('begin','end');
        //echo \think\Debug::getRangeTime('begin','end');
