...
class Search extends Safe{
    ...
    public function play()
    {
        ...
        if(!$keyword){
            return $this->retErr('search.play',10004);
        }
        $where['p.game_type'] = 1;
        $where['p.has_odds'] = 1;
        $where['p.team_home_name|p.team_guest_name'] = array('like', '%' . $keyword . '%');
        ...
        Log::write('search play $data: '.json_encode($data));
        return $this->retSucc('search.play', $data, '');
    }
