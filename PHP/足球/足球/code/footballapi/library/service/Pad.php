...
//        $this->key = $pad && isset($pad['key']) ? $pad['key'] : '';
    ...
    public function getPonints() {
        ...
        Log::write('getPonints data :'.json_encode($data));
        $result = $this->post($this->points, $data);
        Log::write('getPonints $result :'.json_encode($result));
        if (!$this->hasResultSuccess($result)) {
            ...
            return ['status' => false, 'msg' => $result['payResult']];
        }
        if (!$this->checkResultSign($result['signMsg'], [
            ...
            return ['status' => false, 'msg' => '签名验证失败'];
        }
...
//        ])){return false;}
    ...
    public function logout($userId) {
        ...
        $data['imei'] = $user['imei'];
        $data['uuid'] = $user['uuid'];
        Log::write('logout $data :'.json_encode($data));
        $data = $this->setCallbackPointsData(1, $data);
        Log::write('logout $data 1 :'.json_encode($data));
        $data = $this->sendCallbackPoints($data);
        Log::write('logout $data 2 :'.json_encode($data));
        return $data;
    }
    ...
    private function setCallbackPointsData($type, $data) {
        ...
        Log::write('userData :'.json_encode($this->userData));
...
//            ,$data['type']
        ...
        $result = $this->post($this->points_callback, $data);
        Log::write('logout $result :'.json_encode($result));
        if (!$this->hasResultSuccess($result)) {
